...
def visualize_lidar_frame(pts, ax, fused_dist, action, ttc, radar_dist):
    """Visualize LiDAR with RADAR overlay"""
    ax.clear()
    ...
    # Plot LiDAR points
    ax.scatter(x, y, c=colors, s=3, alpha=0.7, label='LiDAR')
    ...
    # RADAR detection (if valid)
    if radar_dist < SENTINEL_DIST and radar_dist < 50:
        # Plot radar detection as RED X
        ax.plot(radar_dist, 0, 'rx', markersize=15, markeredgewidth=3, 
               label=f'Radar: {radar_dist:.1f}m', zorder=10)
        # Radar cone
        ax.plot([0, radar_dist], [0, 0], 'r--', linewidth=2, alpha=0.5)
    
    # Fused detection (final decision)
    if fused_dist < SENTINEL_DIST and fused_dist < 50:
        # Distance line
        ax.plot([0, fused_dist], [0, 0], 'lime', linewidth=4, alpha=0.9, 
               label=f'Fused: {fused_dist:.1f}m', zorder=9)
        
        # Obstacle marker
        circle = plt.Circle((fused_dist, 0), 1.5, color='red', fill=True, 
                           alpha=0.6, linewidth=3, edgecolor='darkred')
        ax.add_patch(circle)
    ...
    ax.set_title('Sensor Fusion View (LiDAR + Radar)', fontsize=13, fontweight='bold')
...
def main():
    ...
    run_dir = os.path.join(log_base, "20260211_161428_rainy_collision_avoidance")
    ...
    # Try to load radar CSV for raw radar data
    radar_csv = os.path.join(run_dir, "radar_data.csv")
    if os.path.exists(radar_csv):
        radar_df = pd.read_csv(radar_csv)
        print(f"✅ Loaded {len(radar_df)} radar records")
    else:
        radar_df = None
        print(f"⚠️  No radar_data.csv found, will use fusion data only")
    ...
    def get_data(idx):
        if idx < len(fusion_df):
            ...
            radar_dist = f.get('radar_dist_m', SENTINEL_DIST)  # Get radar from fusion CSV
        else:
    ...
    try:
        while current_frame < num_frames and plt.fignum_exists(fig.number):
            ...
            # SUPER FAST PLAYBACK - 2x speed (150 FPS)
            plt.pause(0.0065)
